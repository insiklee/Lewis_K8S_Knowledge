...
쿠버네티스의 api-server 및 etcd 등의 핵심 기능들이 
...
remote: Enumerating objects: 4951, done.
remote: Counting objects: 100% (8/8), done.
remote: Compressing objects: 100% (7/7), done.
remote: Total 4951 (delta 1), reused 5 (delta 1), pack-reused 4943
Receiving objects: 100% (4951/4951), 8.91 MiB | 6.59 MiB/s, done.
Resolving deltas: 100% (3151/3151), done.

**# cd cd kube-bench/**

**# ls**
cfg              Dockerfile.ubi8  hooks              job-eks-stig.yaml  job.yaml    OWNERS
check            docs             integration        job-eks.yaml       LICENSE     README.md
cmd              entrypoint.sh    internal           job-gke.yaml       main.go
codecov.yml      go.mod           job-ack.yaml       job-iks.yaml       makefile
CONTRIBUTING.md  go.sum           job-aks.yaml       job-master.yaml    mkdocs.yml
Dockerfile       hack             job-eks-asff.yaml  job-node.yaml      NOTICE
```
...
git으로 다운로드 받은 kube-bench 디렉터리 안에 들어가면 job.yaml 파일이 있다. 해당 파일을 실행한다.
...
```json
**# pwd**
/home/student/kube-bench

**# kubectl apply -f job.yaml** 
job.batch/kube-bench created
```

### 02. job에 의해 실행된 pod 확인

```json
**# kubectl get pod**
NAME                            READY   STATUS      RESTARTS   AGE
**kube-bench-fqhfz                0/1     Completed   0          45s**
```

### 03. 실행 결과 확인

<aside>
💡 **# kubectl logs {파드명} > kube-bench-$(date +%Y%m%d).log**
...
- **결과확인**
    ...
    ```json
    # kubectl logs kube-bench-fqhfz > kube-bench-$(date +%Y%m%d).log
    
    # cat kube-bench-20221018.log
